#!/bin/bash
# P17d v22: For each permutation dir, compute LD scores (xargs 4-way parallel per chr)
# then run S-LDSC with baseline-LD v2.2. Collect into perm_summary_v22.tsv.
#
# Reads: results/phase17d_v22/perm_NNN/cluster_perm.{1..22}.annot.gz
#        (produced by P17d_v22_optimized_permutation.py)
# Writes: results/phase17d_v22/perm_NNN/cluster_perm.{1..22}.l2.ldscore.gz
#         results/phase17d_v22/perm_NNN/h2_perm_v22.results
#         results/phase17d_v22/perm_summary_v22.tsv
...
if [ ! -d "$BASELINE_V22" ]; then
  echo "ERROR: baseline-LD v2.2 directory not found: $BASELINE_V22"
  echo "Download from Zenodo 10515792 first (see README_v22_perm.md)."
  exit 1
fi

# Snp-list pre-check (produced by P17d_v22_build_snplist.py)
if [ ! -f "/tmp/baselineLD_v22_chr1_snplist.txt" ]; then
  echo "ERROR: /tmp/baselineLD_v22_chr{1..22}_snplist.txt missing."
  echo "Run: python3 P17d_v22_build_snplist.py"
  exit 1
fi
...
echo "[v22 PERM RUN] N_PERM=$N_PERM, BASELINE=$BASELINE_V22"
echo

for p in $(seq 0 $((N_PERM - 1))); do
  pdir="$PERM_BASE/perm_$(printf '%03d' $p)"
  if [ ! -d "$pdir" ]; then
    echo "  perm $p: missing — re-run P17d_v22_optimized_permutation.py $N_PERM"
    continue
  fi

  echo "  perm $p: LD scores (4-way parallel)..."
  ...
  if [ -f $pdir/h2_perm_v22.results ]; then
    # Grab the L2_? row corresponding to C0_perm. With baseline-LD v2.2 (97 cats)
    # plus 1 cluster_perm annotation, C0_perm is L2_97 (or whatever the last L2_ is).
    # Use the LAST L2_ row (the perm annotation is appended last).
...
echo
echo "All done."
echo "Summary file: $PERM_BASE/perm_summary_v22.tsv ($(wc -l < $PERM_BASE/perm_summary_v22.tsv) lines incl. header)"
