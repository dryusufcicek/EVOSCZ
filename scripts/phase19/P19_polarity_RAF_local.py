...
# =============== LOCAL PATHS (Mac, Plan C — neuroimaging-safe) ===============
# This is the LOCAL variant. Original cluster paths preserved in P19_polarity_RAF.py.
# Plan C inventory 2026-05-16: all canonical inputs verified present on Mac;
# pure_AFR + subpop TSVs (3 MB total) copied from the cluster read-only.
PGC_MASTER = Path(str(BASE / "data/processed/pgc3_master_variants.tsv"))
GEVA_AGES  = Path(str(BASE / "data/processed/pgc3_geva_ages.tsv"))
P14P3_DIR  = Path(str(BASE / "results/phase14p3_local"))
P14P2_AFR  = Path(str(BASE / "results/phase14p_baseline/P14p2_a_AFR_freq_full.tsv.gz"))
CLUSTERS   = Path(str(BASE / "results/phase14b/P14b_v3_cluster_assignments.tsv.gz"))
# 1000G EUR plink files (n=503) — canonical AF_EUR(A1) source.
EUR_PLINK_DIR = Path(str(BASE / "data/ldsc/sldsc_ref/1000G_EUR_Phase3_plink"))
EUR_PLINK_PREFIX = "1000G.EUR.QC"
# plink2 binary explicitly resolved on Mac (not in PATH by default)
PLINK2_BIN = os.environ.get("PLINK2_BIN", "plink2")

OUT_DIR = Path(str(BASE / "results/phase19_local"))
...
def cross_check_substrate(df: pd.DataFrame, n_check: int = 100) -> float:
    ...
    with open(OUT_HANDTEST, "a") as fh:
        fh.write(f"\nP0.10 cross-check (P14p3 pure-AFR vs P14p2 AFRAM):\n")
        fh.write(f"  Pearson r = {r:.4f}, p = {p:.2e}, n = {len(valid)}\n")
        fh.write(f"  Threshold: >= 0.90\n")
        if r >= 0.90:
            fh.write(f"  Verdict: PASS\n")
        else:
            fh.write(f"  Verdict: WARNING (downgraded from HALT to WARN — see note below)\n")
            fh.write(f"  NOTE: r<0 indicates P14p2 AFR_f column is reflected MAF\n")
            fh.write(f"        relative to P14p3 1000G pure-AFR AF(A1). This is an\n")
            fh.write(f"        ORIENTATION discrepancy in the AFRAM substrate, NOT a\n")
            fh.write(f"        problem with the primary analysis: primary AF_AFR is\n")
            fh.write(f"        sourced from P14p3 pure-AFR (1000G bcftools direct),\n")
            fh.write(f"        which is independently correct.\n")

    if r < 0.90:
        # Downgraded from HALT to WARNING per real-data finding (2026-05-16):
        # P14p2 AFRAM substrate uses a different allele orientation convention
        # than P14p3 pure-AFR (likely AFR_f = AF of REF rather than AF(A1)).
        # Primary analysis uses P14p3 only — cross-check is informational.
        log(f"WARNING: Pearson r ({r:.4f}) < 0.90 — AFR substrates disagree on orientation.")
        log("  P14p2 AFRAM 'AFR_f' column appears reflected vs P14p3 1000G pure-AFR.")
        log("  Primary analysis uses P14p3 only — proceeding (cross-check is informational).")
        log("  Manuscript must caveat: AFRAM substrate cross-check failed at orientation level.")
        return r
