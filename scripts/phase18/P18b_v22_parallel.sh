#!/bin/bash
# Phase 18b parallel: Compute missing LD scores 4-way, then S-LDSC sequentially.
set -e
BASE=${EVOSCZ_ROOT}
...
# === [A] LD scores compute (4-way parallel) for young_annot_k2/k4 ===
echo "[A] Computing LD scores for young_annot_k2/k4 (4-way parallel)"

compute_ld() {
  local k=$1
  local chr=$2
  local dir=$BASE/results/phase14b/young_annot_k${k}
  if [ -f $dir/young.${chr}.l2.ldscore.gz ]; then return; fi
  python3 $LDSC --l2 \
    --bfile ${PLINK_EUR}${chr} \
    --ld-wind-cm 1 \
    --annot $dir/young.${chr}.annot.gz \
    --print-snps /tmp/baseline_chr${chr}_snplist.txt \
    --out $dir/young.${chr} \
    > $OUT/logs/k${k}_chr${chr}.log 2>&1
}
export -f compute_ld
export BASE LDSC PLINK_EUR OUT

# Run k=2 and k=4 chr 1-22 in 4-way parallel (44 total)
for k in 2 4; do
  for chr in {1..22}; do
    echo "$k $chr"
  done
done | xargs -n 2 -P 4 bash -c 'compute_ld "$@"' _

echo "  LD scores done."

# === [B] S-LDSC runs (sequential) ===
echo ""
echo "[B] S-LDSC v2.2 sensitivity battery"
...
YOUNG_K2=$BASE/results/phase14b/young_annot_k2/young.
YOUNG_K4=$BASE/results/phase14b/young_annot_k4/young.

run_sldsc_v22() {
  local tag=$1; local sumstats=$2; local cluster=$3
  local baseline=$4; local wld=$5; local frq=$6; local extra_flags="${7:-}"
  if [ -f "$OUT/$tag.results" ]; then echo "  $tag: SKIP"; return; fi
  python3 $LDSC \
    --h2 $sumstats \
    ...
    --out $OUT/$tag > $OUT/logs/$tag.run.log 2>&1
  if [ -f "$OUT/$tag.results" ]; then echo "  $tag: done"; else echo "  $tag: FAILED"; fi
}
...
run_sldsc_v22 "SCZ_EUR_k2_v22"         $SCZ_EUR    $YOUNG_K2     $V22_EUR $WLD     $FRQ
run_sldsc_v22 "SCZ_EUR_k4_v22"         $SCZ_EUR    $YOUNG_K4     $V22_EUR $WLD     $FRQ
...
ls $OUT/*.results | wc -l | xargs echo "Total .results files in phase18_v22:"
